...
det diag(Y x) == \<y, x\> because \<a,x\>\<b,x\> == \<A x + b, x\> == \<b, x\> + \<&Lambda;x', x'\> == \<b, x\> + Sum &lambda;\_k\<u\_k,x\>\<u\_k,x\> == \<b, x\> + Sum &lambda;\_k\<u\_k, x\> == \<c,x\>, recursive (in first digit meaning, so accuracy is the matter).

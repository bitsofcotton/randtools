...
# General Tips 4
Otherwise, suppose x in {0, 1}^{n0}, Xor\_k And\_m Xor\_n x\_{k,m,n}\*x\_n == any operation on them (And Xor ... is pattern match for 1 pattern, (a xor b) xor (a and b) == (a or b)). So with op == Sum\_k det diag (X\_k x) == det(Y diag(x)). (because x in {0, 1}, x_0:=1, taylor series are in the determinant because all combinations are in them, then, Sum(det(X\_k diag(x))) == Sum\_sigma Pi\_m x\_sigma(m) sgn(sigma) (Pi\_k a^(k)\_m,sigma(m)).).

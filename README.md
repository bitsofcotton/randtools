...
# General Tips 4
Otherwise, suppose x in {0, 1}^{n0}, Xor\_k And\_m Xor\_n x\_{k,m,n}\*x\_n == any operation on them (And Xor ... is pattern match for 1 pattern, (a xor b) xor (a and b) == (a or b)). So with op == Sum\_k det diag (X\_k x) == sum^n0 det(diag(Y x)), this is done by general eigen decomposition {X\_0, X\_1} == {U R\_0 V, U R\_1 V}, decompose 2 of them, then, all of them.  

...
# General Tips 4
If we deal with diag(X\_k x), X\_k in R^{n\*n}, x in {2, 1}^n, Sum\_k det diag(X\_k x) = 2^diag(Sum lb(det(X\_k x))) in F\_2, det(X\_k x) == And\_m Xor\_n x\_{m,n}\*x\_n, lb((Xx)+(Yx))=\[...,||x\_k||cos&theta;||y\_k||cos&psi;,...\]=\[...,\<x'\_k+y'\_k,x\>,...\] (||x||==1). So any function on {2, 1}^n to {2, 1}^n is described as {Y\_k}\_{k=1}^n subset F\_2^{n\*n} s.t. det diag Y(x) = det diag ((\<y\_k,x\>))\_k.

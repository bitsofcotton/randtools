...
(with int, a xor b = a + b, a and b = (a + b) / 2 any first order operations that results N operations can be described as a matrix operations on the bits in int, A in N^{n\*n}: y = Ax (each mod 2).)

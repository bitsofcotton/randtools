...
However, in counting amount meanin, it's only a F-matrix or R^(4n) matrix they represent and complement to any of the data streams.
...
So there might exists {R,R,f} combinations might represents {vector, continuous fourier, some of the vector fields} or {N,f,g} ((theories around N, many approaches not in unique 3 pillars))???, {R,f,g} ((theories around transcendental, might have matrix exp, matrix log with super geometry series in matrix form))??, combinations analogy to them might describes outside of ours well.

However, in elementary descriptions with shallow copying, we're betting 'What came first? The data or structure?' structure infinitely continues.
So our start point is binary: uniqueness of the pillar (structure started) or 3 or more pillars the outside and inside the unique pillar (this is data started).

Might freeze this repository with this.

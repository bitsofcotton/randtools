...
We should test catgp 5 ... chain next.

So this might means our machine is infected from deep inside the system (because no suspicious binary with plain possible minimal system with system random condition with system compiler compiled .cc files with only python, tee pipe output says so).
...
Either, the catgp 3 result only results better with cr.py g \[12\] case a little more inspectation is needed.

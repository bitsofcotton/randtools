...
However, we're in the infection the result controlled into *false positive* condition (this is also observed as working directory to upload temporary directory cp command result returns something FTS_... error but strerror returns Undefined: 0 condition.), we cannot continue to improve our predictors when we test them in this machine.
